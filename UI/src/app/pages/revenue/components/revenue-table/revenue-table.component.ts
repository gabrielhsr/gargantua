...
export class RevenueTableComponent implements OnInit {
		...
		this.periodSubject
			...
			.subscribe((res) => {
				if (res?.isSuccess) {
					...
					this.revenueService.sortOption.next(this.revenueService.sortOption.value);
				}

				this.revenueLoading = false;
			});

		this.revenueService.sortOption.subscribe(option => option ? this.sort(option) : EMPTY);
	}
	...
	public sort(category: SortOption) {
		this.periodRevenue.data = this.periodRevenue.data.sort((revenueA, revenueB) => {
			const itemA = sortingRevenueDataAccessor(revenueA, category.value);
			const itemB = sortingRevenueDataAccessor(revenueB, category.value);

			if (itemA && itemB) {
				return category.order === 'asc' ? (itemA < itemB ? -1 : itemA > itemB ? 1 : 0) : (itemA > itemB ? -1 : itemA < itemB ? 1 : 0);
			}

			throw new Error(`Key named '${category.value}' not finded in object of type 'Revenue'!`);
		});
	}
}
